...
if (!empty($_POST['transfer']) && $_POST['transfer'] == 'upload' && !empty($_FILES['excel']['tmp_name'])) {
    ...
    $handle = fopen($file, "r");
    if ($handle === false) {
        die('<div class="alert alert-danger">Gagal membuka file CSV!</div>');
    }
    $success = $fail = 0;
    $row     = 0;
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        if (empty($email) || empty($name)) {
            echo "<li>Baris $row: Skip - email atau name kosong</li>";
            $fail++;
            continue;
        }
        if ($db->getOne("SELECT id FROM interns WHERE email = '".addslashes($email)."'")) {
            echo "<li>Baris $row: Skip - Email <b>$email</b> sudah terdaftar</li>";
            $fail++;
            continue;
        }
        ...
        if ($db->Execute($q)) {
            echo "<li>Baris $row: <b>$name</b> berhasil ditambahkan</li>";
            $success++;
        } else {
